...
class Xmodel
{
     ...
     * @return mixed 结果
     */
    public function getCountByParams($params, $otherSql=""){
        $stmt = $this->db->select($this->tableName, array("count(*)"), $this->db->paramSqlWhere($params, $otherSql));
    ...
    /**
     * 根据参数查找部分参数
     * @param $findParams 查询的参数名数组
     * @param $params 参数键值对
     * @param string $otherSql 其他语句
     * @return mixed 结果models
     */
    public function findManyParamsByParams($findParams, $params, $otherSql=""){
        $stmt = $this->db->select($this->tableName, $findParams, $this->db->paramSqlWhere($params, $otherSql));
        $stmt = $this->db->getBindStmt($stmt, $params);
        $results = $this->db->getResult($stmt);
        $models = array();
        foreach($results as $result){
