 ...
 */
function Admin_commodity_comm_create() {
    SingletonInheritor.implement(this);
}
SingletonInheritor.declare(Admin_commodity_comm_create);

pr(Admin_commodity_comm_create).init = function () {
    console.log("admin_commodity_comm_create");
    ...
    oBtn.onclick = function () {
        ...
        $("<span>属性</span><input class='paramter_shu' type='text'/>" +
            "<span>参数</span><input class='paramter_can' type='text'/>" +
            "<a style='border:1px solid;cursor: pointer;margin-left: 20px;'>删除</a>").appendTo(oLi);
    ...
    $("#admin_commodity-comm_create-form").submit(this.submit);
    this.clickImg();
};
 ...
 *  表单提交
 */
pr(Admin_commodity_comm_create).submit = function () {
    //拼接属性值并传到后台
    var dataJson = "";
    var paramter_shu = "";
    var paramter_can = "";
    ...
    if ($(".comm_create_name input").val() == "") {
        ...
        var options = {
            type: "POST",
            url: "index.php?controller=admin_commodity&action=comm_create_submit",
        ...
        $("#admin_commodity-comm_create-form").ajaxSubmit(options);
    }
    return false;
};
 ...
 * 设置图片的点击
 */
pr(Admin_commodity_comm_create).clickImg = function () {
    ...
    addpic_button.onclick = function () {
        ...
        var $img = $("<img>").attr("src", "")
                             .attr("width", "90px")
                             .attr("height", "90px").appendTo(new_picdiv);
        var $file = $("<input>").attr("type", "file")
                                .attr("name", "image[]")
                                .css("display", "none").appendTo(new_picdiv);
